"""
Enhanced CatBoost Model with Additional Feature Engineering
- 기존 피쳐 + 새로운 피쳐 추가
- Output: submission_model6_catboost_enhanced.csv, oof_model6_catboost_enhanced.csv
"""
...
def main():
    print("="*60)
    print("Enhanced CatBoost Model with Additional Features")
    print("="*60)
    ...
    print(f"\nCatBoost Enhanced Mean CV F1: {np.mean(fold_scores):.4f}")
    ...
    oof_df.to_csv(cfg.OUTPUT_DIR / "oof_model6_catboost_enhanced.csv", index=False)

    # Save submission
    submission = pd.DataFrame({cfg.ID_COL: test_ids, cfg.TARGET: test_pred})
    submission.to_csv(cfg.OUTPUT_DIR / "submission_model6_catboost_enhanced.csv", index=False)

    # Save test probabilities
    prob_df = pd.DataFrame({"ID": test_ids, "prob": test_prob_mean})
    prob_df.to_csv(cfg.OUTPUT_DIR / "prob_model6_catboost_enhanced.csv", index=False)

    print(f"\nSaved: submission_model6_catboost_enhanced.csv")
