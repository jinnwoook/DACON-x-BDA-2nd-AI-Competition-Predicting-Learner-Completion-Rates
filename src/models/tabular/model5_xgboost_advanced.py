"""
SOTA Model 5: XGBoost Advanced with Feature Engineering
- 피처 엔지니어링 적용
- Output: submission_model5_xgboost.csv, oof_model5_xgboost.csv
"""
...
def main():
    print("="*60)
    print("SOTA Model 5: XGBoost Advanced with Feature Engineering")
    print("="*60)
    ...
    print(f"\nXGBoost Mean CV F1: {np.mean(fold_scores):.4f}")
    ...
    oof_df.to_csv(cfg.OUTPUT_DIR / "oof_model5_xgboost.csv", index=False)

    # Save submission
    submission = pd.DataFrame({cfg.ID_COL: test_ids, cfg.TARGET: test_pred})
    submission.to_csv(cfg.OUTPUT_DIR / "submission_model5_xgboost.csv", index=False)

    # Save test probabilities
    prob_df = pd.DataFrame({"ID": test_ids, "prob": test_prob_mean})
    prob_df.to_csv(cfg.OUTPUT_DIR / "prob_model5_xgboost.csv", index=False)

    print(f"\nSaved: submission_model5_xgboost.csv")
