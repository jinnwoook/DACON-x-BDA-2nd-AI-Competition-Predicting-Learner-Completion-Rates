"""
SOTA Model 1: KoELECTRA-NSMC on bert_train_data.csv
- 원본 텍스트 데이터로 학습
- Output: submission_model1_bert.csv, oof_model1_bert.csv
"""
...
class Config:
    ...
    TRAIN_DATA = "bert_train_data.csv"
    TEST_DATA = "bert_test_data.csv"
...
def search_best_threshold(y_true, y_prob, pos_cap=0.70, step=0.005):
    ...
    if best_f1 < 0:
        ...
        best_pos = float(y_pred_default.mean())
...
def train_one_fold(fold, tr_idx, va_idx, texts, labels, test_texts, tokenizer, collate_fn):
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        thr, f1c, pos_rate = search_best_threshold(labels[va_idx], va_prob, pos_cap=0.70)
...
def main():
    print("="*60)
    print("SOTA Model 1: KoELECTRA-NSMC on bert_train_data.csv")
    print("="*60)

    train_df = pd.read_csv(cfg.DATA_DIR / cfg.TRAIN_DATA, encoding='utf-8-sig')
    test_df = pd.read_csv(cfg.DATA_DIR / cfg.TEST_DATA, encoding='utf-8-sig')
    ...
    # Save OOF
    oof_df = pd.DataFrame({
        "ID": train_df["ID"], "prob": oof_prob, "label": y
    })
    oof_df.to_csv(cfg.OUTPUT_DIR / "oof_model1_bert.csv", index=False)

    # Save submission
    submission = pd.DataFrame({cfg.ID_COL: test_ids, cfg.TARGET: test_pred})
    submission.to_csv(cfg.OUTPUT_DIR / "submission_model1_bert.csv", index=False)

    # Save test probabilities
    prob_df = pd.DataFrame({"ID": test_ids, "prob": test_prob_mean})
    prob_df.to_csv(cfg.OUTPUT_DIR / "prob_model1_bert.csv", index=False)

    print(f"\nSaved: submission_model1_bert.csv")
