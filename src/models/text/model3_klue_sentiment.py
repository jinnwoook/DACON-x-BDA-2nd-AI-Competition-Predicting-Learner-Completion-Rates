"""
SOTA Model 3: KLUE-BERT Sentiment on train_detailed.csv
- 감성 분석 사전학습 모델
- Output: submission_model3_klue.csv, oof_model3_klue.csv
"""
...
def main():
    print("="*60)
    print("SOTA Model 3: KLUE-BERT Sentiment")
    print("="*60)
    ...
    oof_df.to_csv(cfg.OUTPUT_DIR / "oof_model3_klue.csv", index=False)

    # Save submission
    submission = pd.DataFrame({cfg.ID_COL: test_ids, cfg.TARGET: test_pred})
    submission.to_csv(cfg.OUTPUT_DIR / "submission_model3_klue.csv", index=False)

    # Save test probabilities
    prob_df = pd.DataFrame({"ID": test_ids, "prob": test_prob_mean})
    prob_df.to_csv(cfg.OUTPUT_DIR / "prob_model3_klue.csv", index=False)

    print(f"\nSaved: submission_model3_klue.csv")
